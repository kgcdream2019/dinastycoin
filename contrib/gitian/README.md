...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/dinastycoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/dinastycoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/dinastycoin-osx-*-build.assert
```
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/dinastyoffreedom/dinastycoin -b dinastycoin 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
