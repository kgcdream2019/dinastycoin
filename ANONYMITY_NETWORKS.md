...
`--proxy` which tells `dinastycoind` the ip/port of a socks proxy provided by a
...
An ISP can passively monitor `dinastycoind` connections from a node and observe when
